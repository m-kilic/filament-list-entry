    ...
    <div data-l="1" class="{{ $l1Class }}">
        @forelse ($items as $itemKey => $itemElement)
            ...
            <div data-l="2" {{ $tagAttributes }} class="{{ $inline ? 'inline-block' : 'block' }}">
                @if ($hasBadge)
                    <{{$tag}} href="{{ $tagHref }}" target="{{ $target }}">
                ...
                @else
                    @if ($icon = $getItemIcon($itemElement))
                        <{{$tag}} href="{{ $tagHref }}" target="{{ $target }}">
                            <x-filament::icon class="w-5 h-5 text-custom-600 inline" :icon="$icon" style="{{ \Filament\Support\get_color_css_variables($getItemIconColor($itemElement), shades: [50, 300, 400, 500, 600]) }}" />
                        </{{$tag}}>
                    @endif

                    <{{$tag}} href="{{ $tagHref }}" target="{{ $target }}" class="inline-block truncate">
                        {{ $getItemLabel($itemElement) }}
